    ...
    <div class="row jinPaiWenAn">
        ...
        <div class="col-md-10 col-md-offset-1 head-search">
            ...
            <div class="search-tiaoJian text-center">
            	<a href="${path}/lxhAdmin/wenan/addWenAn.jsp" class="btn btn-default" target="_blank">添加咨询顾问</a>
            </div>
        </div>
        <div class="col-md-10 col-md-offset-1">
        	
        	<c:forEach items="${wenAnList}" var="wenAn" varStatus="status">
	            <div class="col-sm-12 wenAn-content">
	                <div class="col-sm-5 text-center content-img">
	                    <a href="${path}/wenan/content.shtml?wenAnGuid=${wenAn.guid}">
	                    	<img src="${path}/static/uploadImg/wenan/liebiao/${wenAn.photoName}">
	                    </a>
	                </div>
	                <div class="col-sm-7 content-miaoShu">
	                    <h4>${wenAn.name}</h4>
	                    <p>${wenAn.shenFen}</p>
	                    <p><span class="miaoshu-title">擅长专业：</span>${wenAn.shanChangZuanYe}</p>
	                    <p><span class="miaoshu-title">人气：</span>${wenAn.renQi}</p>
	                    <p>
	                        <span class="miaoshu-title">评价：</span>
	                        <c:forEach  begin="1" end="${wenAn.xinJiPingJia}" >
	                        	<span class="glyphicon glyphicon-star" aria-hidden="true"></span>
							</c:forEach>
	                    </p>
	                    <div class="col-sm-12 text-center">
	                        <a class="btn btn-default" href="${path}/wenan/content.shtml?wenAnGuid=${wenAn.guid}" style="padding-left:20px;padding-right:20px;">去看看</a>
	                        <a class="btn btn-success" href="${path}/wenan/goUpdateWenAn.shtml?wenAnGuid=${wenAn.guid}" style="padding-left:20px;padding-right:20px;">修改</a>
	                        <a class="btn btn-danger delete" id="${wenAn.guid}" style="padding-left:20px;padding-right:20px;">删除</a>
	                    </div>
	                </div>
	            </div>
        	</c:forEach>
...
<script type="text/javascript">
	$(function(){
		$(".delete").click(function(){
			var wenAnGuid = $(this).attr("id");
			if(confirm("确定要删除数据吗")){
				$.post("${path}/wenan/delete.shtml",{"wenAnGuid":wenAnGuid},function(data){
					var msg = data.msg;
					if(msg=="success"){
						$("#"+wenAnGuid).parent().parent().parent().fadeOut("slow");
					}else if(msg=="fail"){
						alert("系统异常，删除失败");
					}
				});
